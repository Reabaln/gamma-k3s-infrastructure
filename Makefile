...
init:
	rm -rf .terraform ssh
	mkdir ssh
	time terraform init -backend-config="bucket=devops-bootcamp-remote-state2-gamma" -backend-config="key=$$TF_NAMESPACE/labs/terraform.tfstate" -backend-config="dynamodb_table=devops-bootcamp-locks2-gamma"
